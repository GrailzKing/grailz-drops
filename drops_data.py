...
function buildTable(){{
  ...
  DROPS.forEach(d=>{{
    ...
    tr.innerHTML=`<td class="date-cell">${{dateDisplay}}</td><td class="time-cell">${{timeDisplay}}</td><td class="name-cell">${{d.name}}</td><td><span class="cat-badge cat-${{sl}}">${{d.cat}}</span></td><td class="source-cell">${{u1}}${{u1&&u2?' ':''}}${{u2}}</td>`;
